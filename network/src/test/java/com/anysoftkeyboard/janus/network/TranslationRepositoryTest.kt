...
class TranslationRepositoryTest {
  ...
  fun `getTranslation_returnsCachedTranslationWhenAvailable`() = runTest {
    ...
    coEvery { translationDao.findTranslation(sourceWord, sourceLang, targetLang) } returns
        cachedTranslation
  ...
  fun `getTranslation_fetchesFromNetworkAndCachesWhenNotAvailable`() = runTest {
    ...
    val targetLang = "es"
    val networkResponse =
        SearchResponse(query = Query(search = listOf(SearchResult("Hola", "Spanish for hello"))))
